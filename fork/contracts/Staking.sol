...
contract SITHStaking is Ownable {
    ...
    event LogForfeit(address indexed recipient, uint256 DARKAmount, uint256 SITHAmount);
    ...
    /**
        @notice stake SITH to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        SITH.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( DARK.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        DARK.safeTransfer( address(warmupContract), _amount );
        emit LogStake(_recipient, _amount);
        return true;
    }

    /**
        @notice retrieve DARK from warmup
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = DARK.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit DARK in warmup and retrieve SITH
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint DARKBalance = DARK.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  DARKBalance);
        SITH.safeTransfer( msg.sender, info.deposit);
        emit LogForfeit(msg.sender, DARKBalance, info.deposit);
    }
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        SITH.safeTransfer( msg.sender, _amount );
        emit LogUnstake(msg.sender, _amount);
    }

    /**
        @notice returns the DARK index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return DARK.index();
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            DARK.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = DARK.circulatingSupply();
    ...
    /**
        @notice returns contract SITH holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return SITH.balanceOf( address(this) ).add( totalBonus );
    }
