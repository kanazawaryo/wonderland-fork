...
contract SITHBondDepository is Ownable {
    ...
    IERC20 public immutable SITH; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints SITH when receives principle
    ...
    struct Bond {
        uint payout; // SITH remaining to be paid
     ...
     */
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 SITH ( underflow protection )
        ...
        uint balanceBefore = SITH.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) SITH
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            SITH.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == SITH.balanceOf(address(this)), "Not enough SITH to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            SITH.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                SITH.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                SITH.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return SITH.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to SITH supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = SITH.totalSupply();
    ...
    /**
     *  @notice calculate amount of SITH available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or SITH) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != SITH, "NAT" );
